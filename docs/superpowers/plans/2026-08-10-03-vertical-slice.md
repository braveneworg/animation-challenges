...
- `hints` are progressive (concept → shape → exact code), 3 per challenge. `explanation` is markdown with `### Why this works` first, then the pitfall, then the pattern/series comparison. `estimatedMinutes`: 5–8 novice, 10–15 intermediate; `graderTimeoutMs` omitted unless the grader samples frames (then `10_000`).
