...
- Modify: `src/challenges/registry.ts` (retrofit its catch from `String(error)` to `safeString(error)`)
- Test: `src/runner/transpile.test.ts`, `src/runner/safe-string.test.ts`, `src/challenges/registry.test.ts` (one added test)
...
- [ ] **Step 8: Retrofit the registry's catch — buildRegistry's "never throws" becomes literally true**

Plan 01's review documented this gap and carried it forward: `buildRegistry`'s `catch` in
`src/challenges/registry.ts` calls `String(error)`, which itself throws for a value with a
poisoned `toString` — the exact hazard `safeString` exists to neutralize. It fails closed (needs a
hostile first-party author), but this task creates the cure in the same repo, so the known
instance gets fixed here. A repo-wide `grep -rn "String(error)" src/ scripts/` finds exactly this
ONE site — do not hunt for others.

First the failing test. Add to the `describe('buildRegistry', ...)` block in
`src/challenges/registry.test.ts` (it uses `it`, matching the file's convention):
...
Run: `pnpm test:unit src/challenges/registry.test.ts`
Expected: FAIL — the new test throws out of `buildRegistry` (the `String(error)` call re-throws),
proving the "never throws" contract was not yet literally true.

Then the fix in `src/challenges/registry.ts`: add the import

```ts
import { safeString } from '@/runner/safe-string';
```

and change the catch's message interpolation from `${String(error)}` to `${safeString(error)}`
(the surrounding line is otherwise unchanged):
...
Run: `pnpm test:unit src/challenges/registry.test.ts`
Expected: PASS — all existing registry tests plus the new one. This is the first time
`buildRegistry`'s accumulate-don't-throw contract holds for every input.

- [ ] **Step 9: Mutation check — the JSX test detects a broken transform**
...
git add package.json pnpm-lock.yaml src/runner/types.ts src/runner/safe-string.ts src/runner/safe-string.test.ts src/runner/transpile.ts src/runner/transpile.test.ts src/challenges/registry.ts src/challenges/registry.test.ts
...
- `runtime: 'react'` → `App.tsx` is the required entry; its **default export** must be a plain function component. [Amended during execution: `prepareSubmission` only checks that `App.tsx` exists as the entry file — whether its default export is a plain function component is undecidable statically (a bound function, for example, passes any static check that would try to detect it). That convention is instead enforced at render time by Task 10's mount error boundary.]
...
- Diagnostics accumulate — a submission with three broken files reports all three. [Amended during execution: this is staged, not flat. Per-file diagnostics (transpile failures, import-scan problems) accumulate fully across every file. The link-validate probe, however, only runs when zero earlier diagnostics exist — probing a module graph that already contains transpile-failed files produces misleading cascade diagnostics (e.g. spurious "cannot resolve import" errors for imports that would otherwise have resolved), the same reasoning behind TypeScript's and Vite's staged diagnostic reporting.]
...
[Amended during execution: the four snippets below (`react.ts`, `react-jsx-runtime.ts`,
`react-dom.ts`, `react-dom-client.ts`) shipped with explicit named-export lists in place of
`export * from '...'` — React ships CJS, so `export =` typings fail TS2498 and Vite's dev CJS
interop rejects `export *`. `vendor/motion.ts` and `vendor/motion-react.ts` further below are
unaffected: `motion` ships ESM and needs no such workaround.]
...
export default defineConfig({
  ...
  build: {
    rollupOptions: {
      // [Amended during execution: added `preserveEntrySignatures: 'strict'` — without it,
      // Rolldown tree-shakes the vendor entry chunks down to zero exports, since nothing in the
      // build graph imports them (they're reached only through sandbox.html's import map at
      // runtime).]
...
**Files:**
- Create: `src/sandbox/time-controller.ts`, `src/sandbox/sequence.ts`
- Test: `src/sandbox/time-controller.test.ts` (node — `VirtualClock` is pure), `src/sandbox/sequence.test.ts` (node), `src/sandbox/time-controller.browser.test.ts` (Chromium — the spec §6.4 verified scenarios)

**Interfaces:**
- Consumes: nothing from other tasks.
- Produces:
  - `forEachStep(count: number, action: (index: number) => Promise<void | boolean>, index?: number): Promise<void>` (`src/sandbox/sequence.ts`) — the sanctioned sequential-await shape. `eslint(no-await-in-loop)` is an error under this repo's oxlint config and disable comments are banned, so ordered awaits use bounded recursion instead of a loop; an action resolving `false` stops the remaining steps. Consumed here by `stepFrames`, later by `waitForTailwind` (Task 10) and `pointerDrag` (Task 12); Plan 03 re-exports it.
...
- [ ] **Step 1: Write the failing VirtualClock and sequence unit tests**

Create `src/sandbox/sequence.test.ts`:
...
Run: `pnpm test:unit src/sandbox/sequence.test.ts src/sandbox/time-controller.test.ts`
Expected: FAIL — modules not found.

- [ ] **Step 3: Implement the modules**

Create `src/sandbox/sequence.ts`:
...
Two mutations for `sequence.ts`:

3. **Reversed order** — temporarily swap the two awaits so recursion runs before the action (`await forEachStep(count, action, index + 1); await action(index); return;` in place of the current body after the base case). Expected: the in-order test FAILS reading `[3, 2, 1, 0]`. Restore.
4. **Off-by-one base case** — temporarily change `index >= count` to `index > count`. Expected: the zero-count test FAILS (`action(0)` ran once). Restore.

- [ ] **Step 8: Verify and commit**

```bash
pnpm format && pnpm verify
git add src/sandbox/sequence.ts src/sandbox/sequence.test.ts src/sandbox/time-controller.ts src/sandbox/time-controller.test.ts src/sandbox/time-controller.browser.test.ts
...
**Interfaces:**
- Consumes: vendor modules exist (Task 7) — but these files import `react` / `react-dom/client` bare; Vite resolves them to the same underlying chunks, so the harness and user code share one React instance. `forEachStep` (Task 8) for the probe-poll loop.
- Produces:
...
**Interfaces:**
- Consumes: `AssertionDetail`/`AssertionRecord` (Task 1), `safeString` (Task 1), `SandboxEnvironment` (Task 5), `TimeController`/`installTimeController`/`forEachStep` (Task 8), `SIMULATED_HOVER_CLASS` (Task 9).
- Produces:
...
- The iframe is created with `sandbox="allow-scripts allow-same-origin"`, offscreen but rendered (never `display:none` — it kills layout and animations). The offscreen placement is inline styles (`position: fixed; left: -10000px; top: 0`) applied once at creation; an embedder that owns `options.container` may restyle the iframe in place to make it visible — SandboxFrame never re-applies placement styles, and `setEnvironment` rewrites only `width`/`height`. Never re-parent the iframe: moving it in the DOM reloads the document.
...
- For every registry challenge it enforces rule 3 (starter AND solution `prepareSubmission(...).ok`) and grader-file ⇔ gradeMode. For every challenge with `gradeMode !== 'rubric'` — auto AND hybrid alike — it enforces rule 5 (solution report: zero failing assertions, no throw, no timeout, `passed`) and rule 6 (starter report: `passed === false` with a hinted failure or a throw); a hybrid's auto portion must discriminate on its own. Rubric challenges are subject to rule 3 and the must-NOT-have-a-grader check only.
...
- `src/sandbox/sequence.ts` exports `forEachStep(count, action, index?)` — the sanctioned sequential-await shape; `eslint(no-await-in-loop)` is an error repo-wide.
